...
class DexFile {
 ...
 private:
  ...
  const uint8_t* const begin_;//内存中头指针

  // The size of the underlying memory allocation in bytes.
  const size_t size_;//在内存大小

  // Typically the dex file name when available, alternatively some identifying string.
  //
  // The ClassLinker will use this to match DexFiles the boot class
  // path to DexCache::GetLocation when loading from an image.
  const std::string location_; //文件名

  const uint32_t location_checksum_; //头中校验和

  // Manages the underlying memory allocation.
  std::unique_ptr<MemMap> mem_map_;

  // Points to the header section.
  const Header* const header_;//头指针

  // Points to the base of the string identifier list.
  const StringId* const string_ids_;//base + header_->string_ids_off_

  // Points to the base of the type identifier list.
  const TypeId* const type_ids_;//base + header_->type_ids_off_

  // Points to the base of the field identifier list.
  const FieldId* const field_ids_;//base + header_->field_ids_off_

  // Points to the base of the method identifier list.
  const MethodId* const method_ids_;//base + header_->method_ids_off_

  // Points to the base of the prototype identifier list.
  const ProtoId* const proto_ids_;//base + header_->proto_ids_off_

  // Points to the base of the class definition list.
  const ClassDef* const class_defs_;//base + header_->class_defs_off_
  ...
  // null.
  mutable const OatDexFile* oat_dex_file_;//加载来自oat时OatDexFile的指针
