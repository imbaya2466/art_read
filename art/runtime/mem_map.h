...
class MemMap {
 public:
  ...
  //
  // On success, returns returns a MemMap instance.  On failure, returns null.
  //参数:dex大小、标志、标志、fd、0、false、文件路径、错误
  static MemMap* MapFile(size_t byte_count,
                         ...
                         const char* filename,
                         std::string* error_msg) {
      //参数：null、dex大小、标志、标志、fd、0、false、false、文件路径、错误。
 ...
 private:
  ...
  ////加载到内存，参数:null、页面对齐字节数、标志、标志、fd、0、false
  ...
  uint8_t* begin_;  // Start of data. May be changed by AlignBy.//数据开始地址
  size_t size_;  // Length of data. //数据大小
