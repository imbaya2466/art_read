...
static jlongArray ConvertDexFilesToJavaArray(JNIEnv* env,
  ...
  for (size_t i = 0; i < vec.size(); ++i) {
    long_data[kDexFileIndexStart + i] = reinterpret_cast<uintptr_t>(vec[i].get());//智能指针的get返回保存的指针
  }
//复制回java数组并释放c数组
...
static const DexFile* CreateDexFile(JNIEnv* env, std::unique_ptr<MemMap> dex_mem_map) {
  ...
  std::string error_message;
  //从内存构建DexFile对象
  //参数:匿名名字(内存区间)、0、MemMap指针、true、true、错误返回
...
//dex的绝对路径(可能是zip等含有多个dex的文件)、null、0、loader-来源于classloader的this、elements
static jobject DexFile_openDexFileNative(JNIEnv* env,
  ...
  std::vector<std::unique_ptr<const DexFile>> dex_files;//vector存DexFile的指针
