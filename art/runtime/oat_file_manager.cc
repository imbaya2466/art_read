...
//dex文件路径c字串、classloader、dex_elements节点数组、const OatFile**返回、错误信息返回
std::vector<std::unique_ptr<const DexFile>> OatFileManager::OpenDexFilesFromOat(
  ...
  //初始化oat_file_assistant用于oat资源处理
  ...
  //从磁盘获取oat文件 ，GetBestoatFile加载oat到内存，返回OatFile的智能指针，release
  ...
  if (oat_file != nullptr) {
    ...
    if (accept_oat_file) {
	  ...
	  //注册oat文件，将oat_file插入到oat_files_表中
