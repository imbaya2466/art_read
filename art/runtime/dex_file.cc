...
////参数:匿名名字(内存区间)、0、MemMap指针、true、true、错误返回-调用来自CreateDexFile从内存中构建dex
...
//还是调用OpenCommon
//参数：map地址、大小、名字、0、null、true、true、错误返回
...
//dex名字、dex名字、true、错误返回、DexFile返回
...
//句柄、文件名、true、true、错误返回。这里是正式读取了！！！
std::unique_ptr<const DexFile> DexFile::OpenFile(int fd,
  ...
  std::unique_ptr<MemMap> map;//MemMap指针
  {
  //获取文件信息，检测
    File delayed_close(fd, /* check_usage */ false);
    struct stat sbuf;
    memset(&sbuf, 0, sizeof(sbuf));
    if (fstat(fd, &sbuf) == -1) {//获取文件信息
	...
	//使用mmap映射内存，参数:dex大小、标志、标志、fd、0、false、文件路径、错误
	//这个函数定义在mem_map.h中,返回MemMap指针
...
//构造dex头，直接将map头指针强转即可。
  const Header* dex_header = reinterpret_cast<const Header*>(map->Begin());

	//从内存打开dex，和内存加载一致了
	//参数:数据开始地址、数据大小、文件名、dex头的checksum、null、true、true
  std::unique_ptr<DexFile> dex_file = OpenCommon(map->Begin(),
                                                 map->Size(),
                                                 location,
...
//参数:数据开始地址、数据大小、文件名、dex头的checksum、null、true、true-来自DexFile::OpenFile
////参数：map地址、大小、名字、0、null、true、true、错误返回-来自DexFile::Open
//从内存中的数据构建DexFile对象。
